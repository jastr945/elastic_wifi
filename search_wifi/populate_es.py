...
def get_wifi():
    """Extracts WiFi locations from a website and saves them into a file."""
    ...
    for r in result:
        ...
        try:
            ...
            f.write(doc)
            print("{} was saved into a textfile".format(addresscut[0]))
        except AttributeError:
            print("Couldn't convert this one: {}".format(address_total))

    f.close()

    return("Finished saving locations.")
...
def populate_es():
    ...
    for line in f:
        ...
        lon = ((line.split("of America ")[1]).split(" ")[1]).split("\n")[0]
        doc["address"] = address
